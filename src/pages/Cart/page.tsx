...
interface CartItem { id: number; name: string; breed: string; img: string; fee: number; }
    ...
    <section className="px-9 py-10 min-h-[calc(100vh-160px)] max-sm:px-4 max-sm:py-6">
      ...
      {items.length === 0 ? (
        <div className="text-center py-20">
          ...
          <p className="text-secondary mb-6">Browse our available pets and add them to your cart.</p>
          <Link to="/pets"
            className="inline-block px-6 py-3 bg-brand-primary text-white font-bold rounded-[10px]
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-4 mb-8">
            {items.map(item => (
              <div key={item.id}
                className="flex items-center gap-4 bg-card border border-border-token
                           rounded-[10px] p-4 shadow-sm
                           hover:shadow-md transition-shadow duration-300">
                <img src={item.img} alt={item.name}
                  className="w-20 h-20 object-cover rounded-[6px] flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-base text-primary">{item.name}</h3>
                  <p className="text-sm text-secondary">{item.breed}</p>
                  <p className="text-xs text-muted mt-1">Adoption fee</p>
                </div>
          ...
          {/* Summary */}
          <div className="bg-card border border-border-token rounded-[10px] p-6 shadow-sm">
            <h2 className="text-lg font-bold text-primary mb-4">Order Summary</h2>
            <div className="space-y-2 mb-4">
              {items.map(i => (
                <div key={i.id} className="flex justify-between text-sm">
                  <span className="text-secondary">{i.name} adoption fee</span>
                  <span className="font-semibold text-primary">${i.fee}</span>
                </div>
              ))}
              <div className="border-t border-border-token pt-3 flex justify-between font-bold text-base">
                <span className="text-primary">Total</span>
                <span className="text-brand-primary">${total}</span>
              </div>
            </div>
            <Link to="/adopt"
              className="block w-full text-center py-3 bg-brand-primary text-white font-bold
                         rounded-[6px] no-underline transition-all duration-300
