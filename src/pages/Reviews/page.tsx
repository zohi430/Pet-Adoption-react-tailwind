...
function Stars({ rating }: { rating: number }) {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map(s => (
        <span key={s} className={s <= rating ? 'text-brand-fourth' : 'text-gray-300'}>★</span>
      ))}
    </div>
  );
}
  ...
  return (
    <section className="px-9 py-10 min-h-[calc(100vh-160px)] max-sm:px-4 max-sm:py-6">
      ...
      <div className="text-center mb-10">
        ...
        <div className="inline-flex items-center gap-2 bg-[var(--bg-card)] border border-[var(--border-muted)]
                        rounded-full px-5 py-2 shadow-sm">
          <span className="text-2xl font-extrabold text-brand-primary">{avg}</span>
          <Stars rating={Math.round(Number(avg))} />
          <span className="text-sm text-[var(--text-secondary)]">({reviews.length} reviews)</span>
        </div>
      </div>

      {/* Write a review */}
      <div className="max-w-xl mx-auto mb-12 bg-[var(--bg-card)] border border-[var(--border-muted)]
        ...
        {user ? (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-semibold mb-1.5 text-[var(--text-secondary)]">Rating</label>
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map(s => (
                  <button key={s} type="button" onClick={() => setForm(f => ({ ...f, rating: s }))}
                    className={`text-2xl bg-transparent border-none cursor-pointer transition-transform hover:scale-110
                                ${s <= form.rating ? 'text-brand-fourth' : 'text-gray-300'}`}>
                    ★
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className="block text-sm font-semibold mb-1.5 text-[var(--text-secondary)]">Pet Adopted</label>
              <input type="text" placeholder="e.g. Buddy the Labrador" value={form.pet}
                onChange={e => setForm(f => ({ ...f, pet: e.target.value }))}
                className="w-full px-4 py-2.5 rounded-xl text-sm border transition-all duration-300 box-border" />
            </div>
            <div>
              <label className="block text-sm font-semibold mb-1.5 text-[var(--text-secondary)]">Your Review</label>
              <textarea rows={3} placeholder="Share your adoption experience..."
                value={form.text} onChange={e => setForm(f => ({ ...f, text: e.target.value }))}
                className="w-full px-4 py-2.5 rounded-xl text-sm border resize-y transition-all duration-300 box-border" />
            </div>
            <button type="submit"
              className="w-full py-2.5 bg-brand-primary text-white font-bold rounded-xl border-none
                         cursor-pointer transition-all duration-300 hover:bg-brand-secondary hover:-translate-y-0.5">
              Submit Review
            </button>
          </form>
        ) : (
          <p className="text-sm text-[var(--text-secondary)]">
            Please{' '}
            <a href="/login" className="text-brand-primary font-bold hover:text-brand-accent transition-colors duration-200">
              login
            </a>{' '}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
        {reviews.map(r => (
          <div key={r.id}
            className="bg-[var(--bg-card)] border border-[var(--border-muted)] rounded-2xl p-6
                       hover:shadow-lg transition-shadow duration-300">
            <div className="flex items-center gap-3 mb-3">
              <img src={r.avatar} alt={r.name} className="w-11 h-11 rounded-full object-cover" />
              <div>
                <p className="font-bold text-sm text-[var(--text-primary)]">{r.name}</p>
